...
- 新增 `Wigs` 独立测试区，公开类目固定为 `Category:33935`，卖家后台完整路径固定为：
  `Personal & Lifestyle / Beauty / Hair Care / Wigs`。
- 假发与封口机使用不同测试批次，重复试采只覆盖各自测试区，不会互相覆盖。
- 兼容 Takealot 对大类目返回 `5000+` 近似总数：以游标自然结束后的实际唯一 PLID 数为准。
- 假发分页上限提高到 400 页，并增加重复游标保护；未自然结束、重复 PLID、详情类目不一致或卖家路径不唯一都会判定失败。
- `POST /api/takealot/market/category-collection-test/wigs` 启动假发试采；同地址 `GET` 查看进度和结果。
- v7.5 的正式重建仍只允许已验证的 397 个 Vacuum Sealers；假发测试数据不会参与或放宽删除保护。

## V7.5 正式类目绑定重建

在 v7.4 已通过的 397 个 Vacuum Sealers 隔离试采基础上，增加受保护的正式商品库重建流程。旧商品、快照和采集类目状态会先按批次归档，再在单一数据库事务中替换为 397 个直接绑定完整卖家类目路径的商品；任何前置或后置校验失败都会整体回滚。

部署不会自动清理数据，正式执行要求 `CATEGORY_ADMIN_TOKEN`、精确确认语句、旧商品数 12,207 和 `MARKET_COLLECTION_ENABLED=false` 同时成立。
...
- 修复 `PS4Controller` 等粘连标题，并补充飞行摇杆、农场摇杆与赛车踏板识别。
- 游戏主机电源和没有安全目标的维修件保留未匹配，不再误归电池包。
- 游戏存储卡读卡器优先匹配 Gaming Memory Cards。
- 继续只生成建议，不自动确认、不修改 Takealot 线上类目；商品采集保持暂停。

## V7.3 Controllers 标题分流

- 对旧类目 `Controllers` 先按商品标题判断完整手柄、游戏主机/游戏棒、保护壳、充电底座、散热器、线材与转接器、键盘、支架及维修零件。
- 强特征商品直接推荐到对应的游戏完整路径；维修零件或只有型号、证据不足的商品保留未匹配，避免被分散到无关类目。
- 规则只生成建议，不自动确认、不修改 Takealot 线上类目。
- 不使用 GPT API；商品采集继续暂停。
