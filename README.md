...
V6.4 使用混合高速监控：优先读取 Takealot 商品页使用的公开 JSON 数据，失败才回退 Chromium；相同 Productline ID 的并发任务会合并，并复用两分钟内的结果。跟卖中商品每 10 分钟进入快速队列，无人跟卖和未找到本店铺商品仍于北京时间每日 10:00 更新。动态竞价调价后的确认会跳过缓存，立即读取最新结果。
